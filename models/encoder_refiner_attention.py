...
class ImageScoreWindowAttention(nn.Module):
    ...
    def forward(
        ...
        if tuple(sam_image_last.shape) != (B, D, H, W):
            raise ValueError(
...
class EncoderRefinerLayer(nn.Module):
    ...
    def _ffn(self, e: torch.Tensor) -> torch.Tensor:
        B, C, D, H, W = e.shape
        e_flat = e.permute(0, 3, 4, 1, 2).reshape(B * H * W, C, D)
        residual = e_flat
        e_flat = self.ffn_norm(e_flat)
        e_flat = self.ffn_fc2(
            self.ffn_dropout(F.gelu(self.ffn_fc1(e_flat)))
        )
        e_flat = residual + self.ffn_dropout(e_flat)
        return e_flat.reshape(B, H, W, C, D).permute(0, 3, 4, 1, 2).contiguous()
