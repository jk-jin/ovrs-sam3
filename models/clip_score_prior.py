...
class ClipScorePriorBuilder(nn.Module):
    """
    Build 18x18 CLIP score prior from dynamic CLIP text and image feature map.
    ...
    Flow:
        clip_image_feat_map -> bilinear to 18x18
        -> text-image dot product -> score_maps_18: [B, C, Q, 18, 18]
        -> Conv2d(Q -> D_score, 7x7) -> score_embed_18: [B, C, D_score, 18, 18]

    Only the 18x18 scale is built; 36x36 and 72x72 are no longer used.
    """
    ...
    def forward(
        ...
        score_embed_18 = score_embed_18_flat.reshape(
            batch_size,
            num_classes,
            self.score_embed_dim,
            self.base_hw,
        ...
        return score_embed_18, score_maps_18.contiguous()
